...
MeshPolicySnapshot ReferenceHarness::capturePolicySnapshot(mesh::Packet *packet,
							   const uint8_t *hash)
{
	MeshPolicySnapshot snapshot = {};
	mesh::GroupChannel channels[4] = {};

	snapshot.filter_recv_flood_packet = filterRecvFloodPacket(packet);
	snapshot.allow_packet_forward = allowPacketForward(packet);
	snapshot.retransmit_delay = getRetransmitDelay(packet);
	snapshot.direct_retransmit_delay = getDirectRetransmitDelay(packet);
	snapshot.extra_ack_transmit_count = getExtraAckTransmitCount();
	snapshot.cad_fail_retry_delay = getCADFailRetryDelay();
	snapshot.search_peers_by_hash = searchPeersByHash(hash);
	snapshot.search_channels_by_hash =
		searchChannelsByHash(hash, channels, 4);
	return snapshot;
}

mesh::DispatcherAction ReferenceHarness::onRecvForTest(mesh::Packet *packet)
{
	return onRecvPacket(packet);
}

mesh::DispatcherAction ReferenceHarness::routeRecvForTest(mesh::Packet *packet)
{
...
MeshPolicySnapshot capture_reference_snapshot(ReferenceEnv &env,
					      ...
					      const uint8_t *hash)
{
	return env.mesh.capturePolicySnapshot(packet, hash);
}
